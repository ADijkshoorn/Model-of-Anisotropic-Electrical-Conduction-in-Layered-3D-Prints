...
if real(Iinp)<0 Iinp=-1*Iinp; end
Z=(Uhig-Ulow)/Iinp;
...
for k=1:Ntrax           %k runs over the number of traxels
    for m=1:Npoints     %m runs over the position in each traxel
        ...
        for q=3:Nlambda U(k,m)=U(k,m)+Alpha(q)*eVec(k,ceil(q/2))*exp(Lambda(q)*zp);
        end
    end
end
...
for k=1:Ntrax           %k runs over the number of traxels
    for m=1:Npoints     %m runs over the position in each traxel
        ...
        for q=3:Nlambda I(k,m)=I(k,m)+Alpha(q)*eVec(k,ceil(q/2))*Lambda(q)*exp(Lambda(q)*zp);
        end
    end
end
